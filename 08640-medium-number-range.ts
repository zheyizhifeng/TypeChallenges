...
type NumberRange<
  ...
  H extends number,
  U extends any[] = [],
  Flags extends boolean = U['length'] extends L ? true : false
> = U['length'] extends H
  ? U[number] | H
  : Flags extends false
  ? NumberRange<L, H, [...U, never]>
  : NumberRange<L, H, [...U, U['length']], Flags>;
/**
 * 以下解法递归的次数太多，会爆栈
 */
...
type AddOne<T extends number> = [...Tuple<T>, any]['length']
type SubOne<T extends number> = Tuple<T> extends [...infer R, any] ? R['length'] : never
type Test = AddOne<100> // 101
type Test1 = SubOne<100> // 99

type NumberRange<L extends number, H extends number> = L extends H
? L
: L |  NumberRange<AddOne<L>, H>
 */
